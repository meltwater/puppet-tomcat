...
define tomcat::instance($ensure="present",
  ...
  elsif $tomcat::params::maj_version == "7" {
  ...
  elsif $tomcat::params::maj_version == "7" and $tomcat::params::type == "package" {
    $catalinahome = $::operatingsystem ? {
  ...
  case $ensure {
    present,installed,running,stopped: {
      ...
      if $sample {
        ...
        #
        file { "${install_dir}/webapps/sample.war":
          ensure  => present,
          owner   => "tomcat",
          group   => $group,
          mode    => '0460',
  ...
  service {"tomcat-${name}":
    ...
    pattern => "-Dcatalina.base=${install_dir}/${name}",
  }
