...
Installs 2 extra components to $tomcat::tomcat_home, required to configuring logging on
...
class tomcat::juli {
  ...
  file { "${tomcat::tomcat_home}/extras/":
    ensure  => directory,
    require => File[$tomcat::tomcat_home],
  }
  ...
  archive::download { "tomcat-juli.jar":
    ...
    digest_type => "md5",
    src_target  => "${tomcat::tomcat_home}/extras/",
    require     => File["${tomcat::tomcat_home}/extras/"],
  }
  ...
  file { "${tomcat::tomcat_home}/bin/tomcat-juli.jar":
    ensure  => link,
    target  => "${tomcat::tomcat_home}/extras/tomcat-juli.jar",
    require => Archive::Download["tomcat-juli.jar"],
  }

  file { "${tomcat::tomcat_home}/lib/tomcat-juli-adapters.jar":
    ensure  => link,
    target  => "${tomcat::tomcat_home}/extras/tomcat-juli-adapters.jar",
